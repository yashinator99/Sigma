...
def validate_first_name(first_name):
    if re.findall('[A-Za-z]{3,15}', first_name) and len(first_name) <=15 and not " " in first_name:
        return True
    else:
        return False
...
def validate_last_name(last_name):
    if re.findall('[A-Za-z]{3,15}', last_name) and len(last_name) <=15 and not " " in last_name:
       return True
    else:
        return False
